...
| `AGENT_CONTEXT_WARN_PERCENT` | `90` | how full the model window may get before `lca agent watch` warns. It **stops** the run outright on real truncation, which is not tunable — a cut prompt has lost its rules and its tools (docs/PROMPT-WINDOW.md) |
