...
restore_webui_volume() {
  ...
  if ! docker_daemon_reachable; then
    warn "The Docker daemon is not responding, so the WebUI volume was NOT restored — nothing was touched and any existing data is intact. $(docker_unreachable_advice). Then re-run ${SCRIPT_DIR}/restore.sh; continuing with the model restore."
    return 0
  fi
